...
export default function TambahProduk() {
  ...
  const { postData, isLoading } = usePostDataUsingFormData("admin/products");
  ...
  const handleDeletedisplayImage = () => {
    var index = formik.values.displayImage.findIndex((element) => {
      return element == choosed;
    });
    if (index > 0) {
      formik.values.displayImage.splice(index, 1);
      formik.values.image.splice(index, 1);
      changeDisplay(0);
    } else {
      formik.values.displayImage.shift();
